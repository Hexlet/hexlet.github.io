---
title: "Сроки обучения"
---

[Перейти на сайт](https://ru.hexlet.io)

# Сроки обучения

> Эта статья поможет понять, какие примерно сроки отводятся на каждый тип задач и в какой момент нужно просить о помощи.

Самое важное в процессе обучения — это регулярность. Залог успеха — вовремя задавать вопросы и не тратить время впустую.

## Программа

Прежде, чем приступить к обучению, рекомендуем ещё раз ознакомиться со своей программой. Зайдите на страницу [Дашборд](https://ru.hexlet.io/my) и убедитесь, что у вас отображается нужная программа. Если вам необходимо выбрать другой курс, то выберите его в разделе [Мои курсы](https://ru.hexlet.io/my/programs) или [Профессии](https://ru.hexlet.io/my/catalog/professions).

На странице [Дашборд](https://ru.hexlet.io/my) изучите, в каком порядке идут модули и что каждый из них раскрывает. Это позволит лучше понять цели обучения и получаемый на каждом этапе результат.

![](/img/docs/img-086.png)

## План обучения

Изучите свою программу и посмотрите, из какого количества модулей и проектов она состоит. Допустим, программа идёт 10 месяцев, имеет 5 модулей и 4 проекта. Тогда рекомендуемый план обучения — это 1 месяц на модуль без проекта и по 2 месяца в среднем на модули с проектом.

Для модуля с проектом эти 2 месяца распределим так:

- 2-4 недели на все курсы до проекта;
- 2-4 недели на решение проекта;
- до 2 недель на доработки проекта по код-ревью, если вы учитесь с наставником.

Это оптимальные сроки, и, чтобы в них уложиться, мы рекомендуем проходить в среднем от 2 до 5 уроков в день. Урок — это теория, тест и практика. Но бывают и уроки, состоящие исключительно из теории.

### Как посмотреть свой прогресс

**Если вы учитесь самостоятельно**, то у вас нет ограничений по времени прохождения программы, но на основании рекомендаций выше вы можете ставить себе дедлайны самостоятельно, чтобы обучение проходило продуктивнее.

**Если вы учитесь на тарифе с поддержкой наставника**, то ваше обучение рассчитано на определенный срок. Чтобы узнать, какая часть программы уже пройдена и укладываетесь ли вы в отведённое время:

1. Зайдите на страницу [«Мои курсы»](https://ru.hexlet.io/my/programs)
2. Найдите вашу программу и нажмите на её название

Откроется **страница с прогрессом** по программе, на которой видно:

- Прогресс прохождения программы в процентах

- Сколько уроков и проектов уже выполнено, а сколько осталось

- Сколько уроков в день нужно проходить, чтобы уложиться в срок

- Дату начала и окончания обучения

![](/img/docs/img-068.png)

Если видите, что не успеваете к дате завершения, — напишите куратору, и он поможет найти решение.

### Уроки

Как понять, что пора обратиться за помощью:

- Если уже 20 минут нет прогресса в решении задачи.
- Когда задача решается, но время решения выходит за 60 минут. Даже самые объёмные задачи в Хекслете не требуют целого часа непрерывного написания кода.

### Проект

Каждый проект закрепляет темы из пройденного модуля, поэтому будьте последовательны, проходите проекты сразу после завершения модуля. На каждый шаг проекта также рекомендуем тратить несколько часов плотной работы и не более 60 минут на самостоятельное преодоление трудностей.

### Что делать, если вы застряли и решить задачу не получается?

Когда вы застряли и в течение часа не можете сдвинуться с места, не видите вариантов решения или не можете справиться с ошибкой, почитайте обсуждения или задайте вопрос [ИИ-помощнику](/docs/practice/chto-takoe-ii-pomoshchnik-i-pochemu-on-mozhet-oshibatsya.md). Если всё равно не получается разобраться, то обратитесь к наставнику. А если вы учитесь самостоятельно, можете задать вопрос в [нашем Сообществе](/docs/community-and-media/telegram-soobshchestvo-heksleta.md).

Главное, чтобы процесс обучения не прерывался. Сидеть часами и думать над задачей, гуглить решения, но не обратиться за помощью — это самый неоптимальный сценарий.

Чтобы правильно сформулировать проблему, с которой вы столкнулись, прочитайте статью [Как правильно задавать вопросы](/docs/learning-support/kak-pravilno-zadavat-voprosy.md).

Если вы учитесь без поддержки наставника, но чувствуете потребность в помощи, рассмотрите вариант перехода на тариф [Расширенный](/docs/formats-and-tarifs/tarifs/tarif-rasshirennyi.md), [Стандартный](/docs/formats-and-tarifs/tarifs/tarif-standartnyi.md), [Оптимальный](/docs/formats-and-tarifs/tarifs/tarif-optimalnyi.md) или [Премиальный](/docs/formats-and-tarifs/tarifs/tarif-premialnyi.md).

## Подсказки на сайте

В процессе обучения вы будете находить различные подсказки:

- элементы интерфейса;
- ссылки на дополнительные материалы;
- самостоятельные задания;
- обсуждения в упражнениях и проектах;
- [база знаний Хекслета](https://hexlet.github.io/).

Используйте их все, чтобы оставаться эффективным.

А в каждой практике есть окно «Подсказка», где объясняется, что делать, если вы застряли — когда и как обращаться за помощью, как посмотреть решение учителя:

![](/img/docs/img-087.png)

## Что делать, если я не успеваю?

Если вы учитесь по Базовой или Премиум подписке, то дедлайнов у вас нет, продлевать подписку можно бесконечно. Срок доступа к контенту можно увидеть [в разделе “Оплата”](https://ru.hexlet.io/account/subscription).

Если вы учитесь с наставником и пропустили несколько дней обучения по каким-то причинам, то не переживайте. Напишите своему куратору. Он проанализирует ситуацию и подскажет, на каком этапе программы вам нужно быть на сегодняшний день.

По ходу обучения всегда есть моменты замедления или ускорения. А зачастую тормозит технический долг и неправильный подход к обучению. Но бывают и жизненные ситуации, которые выбивают из колеи. Мы всегда идём навстречу студентам и находим варианты, как помочь дойти до конца, несмотря на любые трудности. Для перерывов на неделю и более можно оформить [академический отпуск](/docs/learning-process/akademicheskii-otpusk.md).

Поэтому обращайтесь к куратору сразу при появлении проблем, чтобы они не накапливались, как снежный ком.
