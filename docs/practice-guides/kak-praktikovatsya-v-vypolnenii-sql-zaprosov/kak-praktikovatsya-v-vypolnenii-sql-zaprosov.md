...
Мы воспользуемся сервисами, которые позволяют попрактиковаться в выполнении запросов прямо в браузере. Один из таких — [DB Fiddle](https://www.db-fiddle.com/). Это довольно популярный сервис, который поддерживает различные базы данных и не требует регистрации. Так он выглядит:
...
- **Второй вариант.** Если готового скрипта нет, вы можете воспользоваться специальной возможностью — Text to DDL и DB Fiddle поможет преобразовать текст в SQL-скрипт. Нажмите кнопку **Text to DDL**, далее вы сможете дать вашей таблице имя и наполнить ее данными:
